...
def _(mo):
    ...
    **This file is today's session, start to finish.** It is meant to be done
    ...
    In class I stop at a few points and ask the room what a cell is about to do
    before I run it. Nothing about that is graded, and nothing in this file
    depends on having been there.
    """)
    return
...
def _(mo):
    ...
    **The smooth curve is a KDE**, which is a smoothed version of the same
    ...
    Here it is for the happiness score, across all 156 countries.
    """)
    return
...
def _(mo):
    ...
    **Finland is not an outlier**, but the highest value in a distribution wide
    ...
    The same plot of `corruption` looks nothing like it. That column is how much
    **low** perceived corruption contributes to happiness, so a high value means a
    cleaner government.
    """)
    return
